  ...
  const services = [
    {
      ...
      icon: <Home size={32} />,
      description:
        "Find your dream home with our residential property listings.",
      link: "residential",
    },
    {
      title: "Commercial",
      ...
      description:
        "Discover premium commercial properties for your business needs.",
      link: "commercial",
    },
    {
      title: "Shop",
      icon: <Store size={32} />,
      description:
        "Explore retail spaces in prime locations to grow your business.",
    },
    {
      title: "Office",
      icon: <Briefcase size={32} />,
      description:
        "Premium office spaces designed for productivity and growth.",
    },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            <Link to={service.link}>
              <div
                key={index}
                className="border border-[#CC9D16] rounded-lg p-6 transition-all duration-300 hover:bg-[#CC9D16] hover:text-black cursor-pointer"
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
              >
                <div
                  className={`flex justify-center mb-4 ${
                    hoveredIndex === index ? "text-black" : "text-[#CC9D16]"
                  }`}
                >
                  {service.icon}
                </div>
